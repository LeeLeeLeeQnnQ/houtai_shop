...
export const setStoreId = (storeid) => {
  window.localStorage.setItem('gdjumaostoreid', storeid)
}
export const getStoreId = () => {
  const storeId = window.localStorage.getItem('gdjumaostoreid')
  return storeId
}
...
export const getMenuByRouter = function(list, access, arr){
  list.forEach( function(item,index) {
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      ...
      if ( hasChild(item) && showThisMenuEle(item, access) ) {
        obj.children = [];
         getMenuByRouter(item.children, access ,obj.children)
      }
      let k = showThisMenuEle(item, access)
      if(!!k){
        arr.push(obj)
      }
    }
